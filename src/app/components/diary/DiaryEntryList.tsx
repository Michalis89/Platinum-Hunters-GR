...
function DiaryEntryListComponent({ entries, activeEntryId, onSelect }: DiaryEntryListProps) {
  if (entries.length === 0) {
    return (
      <div className="animate-in rounded-2xl border border-dashed border-border/80 bg-card/50 p-10 text-center duration-200">
        <BookHeart className="mx-auto mb-4 h-8 w-8 text-muted-foreground" />
        <p className="text-2xl font-medium leading-8 text-foreground">Your quiet space is waiting.</p>
