...
export default async function HobbiesPage() {
  ...
  // Server-side auth check (replaces client-side useSelector)
  const supabase = await createRouteHandlerClient();
  const {
    data: { user: authUser },
  } = await supabase.auth.getUser();

  // Determine filtered sections based on auth and user categories
  let filteredSections = HOBBY_SECTIONS;

  if (authUser) {
    // Fetch full user profile from database to get categories
    const { data: userProfile } = await supabase
      .from('users')
      .select('categories, roles')
      .eq('id', authUser.id)
      .single();

    if (userProfile) {
      const hasAllAccess = hasAnyRole(userProfile, ['admin', 'owner', 'moderator']);
      const userCategories = (userProfile.categories as string[] | null) ?? [];

      if (!hasAllAccess && userCategories.length > 0) {
        filteredSections = HOBBY_SECTIONS.filter(section =>
