...
export default function ProfilePageClient() {
  ...
  const [authUserByUserId, setAuthUserByUserId] = useState<{
    userId: string;
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([articleResult, reviewResult, userStatsResult]) => {
        if (cancelled) {
          return;
        }
        const entriesValue = userStatsResult?.data?.total_backlog;
        setHeroStats({
  ...
  const authUser =
    user && authUserByUserId && authUserByUserId.userId === user.id
      ? authUserByUserId.authUser
      : null;
  const identity = useMemo(
    () => (user ? resolveProfileIdentity(user, authUser) : null),
    [authUser, user],
  );
  const categoryProfileWithInsights = user?.category_profile ?? null;
